...
NexusFabric支持Docker 和 Docker-Compose 部署,在解压发布包后,内部自带 Dockerfile 用于构建镜像
...
NexusFabric 在启动时还需要数据库和DDP部署包才可以真正的进行使用
...
NexusFabric 的所有应用配置文件在 conf 目录下。如果需要使用 NexusFabric 的全部功能，在这之前先了解一下如何进行配置
...
`datasophon.conf` 为快捷配置文件；如果你只想快速体验 NexusFabric 的功能，配置它就足够了。datasophon.conf 本质上是 application-config.yml 中常用配置的快捷方式
